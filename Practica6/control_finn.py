...
class RedPitayaApp():
    def __init__(self, url, name=None):
        ...
        if 'lock_in+pid_harmonic_hf' in self.url:
            self._PERIOD_LEN = 120
        else:
            self._PERIOD_LEN = 2520
    ...
    def set_lpf(self, tau_exponent=14, order=2 , line='ref' ):
        ...
        """
        if not type(tau_exponent)==int:
            warnings.warn(f"tau_exponent is of type {type(tau_exponent)} and should be int")
            tau_exponent = int(tau_exponent)
        ...
        print(f'LPF was set to 8 ns * 2**{tau_exponent} = { int(round(8*2**tau_exponent/1000)) } us , order {order}')
        return 8e-9*2**val
    ...
    def get_lpf(self,line=None):
        ...
        if line is None:
            print('Values of Low Pass Filters in Human readable numbers (order 0 means OFF):')
            ...
            lines = 'ref 1f 2f 3f'.split()
            if type(line)==str and line.lower() in lines:
                line = lines.index( line.lower() )
            ...
            if type(line)==int :
                ...
                raise ValueError("Can't recognise which line you want to query")
    ...
    def get_XY(self,units='V',F=False):
        ...
        self.lock.read_ctrl = 0 # read_ctrl
    ...
    def get_freq(self):
        ...
        """
        return 1.0/(  (self.lock.gen_mod_hp+1)*self._PERIOD_LEN*8e-9  )
    
    def get_period(self):
        """
        Returns the lock-in base modulation period in sec
        """
        return (self.lock.gen_mod_hp+1)*self._PERIOD_LEN*8e-9 
    ...
    def set_freq(self, val ):
        ...
        """
        if not type(val)==int:
            warnings.warn(f"val type is {type(val)} and should be int")
            val = int(val)
    ...
    def get_modulation_amplitud(self,ch):
        """
        Gets the amplitude of modulation over an specific output channel.
        Returns the modulation amplitude in Volts.
            ch   : channel. Can be 'out1' or 'out2' or just the integer 1 or 2
        """
        if type(ch)==int and ch in [1,2]:
            ch = f'out{ch}'
        ...
        val = getattr(self.lock, 'mod_out'+ch[-1] )
    ...
    def set_phase(self,val):
        ...
        The value represents the de-phasing of cos_ref vs cos_1f (and also cos_2f and cos_3f).
        ...
        if val<0 or val>self._PERIOD_LEN-1:
            raise ValueError(f'val wrong value. Values should be from 0 to {self._PERIOD_LEN-1}. You tryed to set: {val}')
        
        self.lock.gen_mod_phase = val
        
        return float(val)/self._PERIOD_LEN*360
    ...
    def get_phase(self):
        ...
        return float(self.lock.gen_mod_phase)/self._PERIOD_LEN*360
